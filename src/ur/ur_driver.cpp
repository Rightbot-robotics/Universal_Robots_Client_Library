...
bool UrDriver::startPayloadEstimation(control::PayloadEstimType command_type, double move_distance, double secondary_move_distance, double move_speed)
{
  if (getVersion().major < 5)
  {
    std::stringstream ss;
  ...
  if (script_command_interface_->clientConnected())
  {
    return script_command_interface_->startPayloadEstimation(command_type, move_distance, secondary_move_distance, move_speed);
  }
  else
  {
